...
func TestGlockApi_HandleUnlock_MissingKey(t *testing.T) {
	g := NewForTest()
...
func TestGlockApi_HandleUnlock_MissingSecret(t *testing.T) {
	g := NewForTest()
...
func TestGlockApi_HandleUnlock_NotLocked(t *testing.T) {
	g := NewForTest()

	w := httptest.NewRecorder()
	r, _ := http.NewRequest("GET", "?key=notLocked&secret=notNeeded", nil)
	g.HandleUnlock(w, r)
...
func TestGlockApi_HandleUnlock_BadSecret(t *testing.T) {
	g := NewForTest()
	...
	// Attempt to unlock using a bad secret
	w = httptest.NewRecorder()
	r, _ = http.NewRequest("GET", fmt.Sprintf("?key=%v&secret=badsecret", key), nil)
	g.HandleUnlock(w, r)
...
func TestGlockApi_HandleUnlock_Positive(t *testing.T) {
	g := NewForTest()
	...
	w = httptest.NewRecorder()
	r, _ = http.NewRequest("GET", fmt.Sprintf("?key=%v&secret=%v", key, lockRes.Extras["secret"]), nil)
	g.HandleUnlock(w, r)
